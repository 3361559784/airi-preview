# CLAUDE CODE 对齐发版说明（Claude Code Patterns for AIRI）

这份文档不是源码摘抄，也不是“Claude Code 模仿计划”。

这不是开发执行手册。这是发版评审说明：用于确认本次发布是否沿着正确边界发版，不是决定“下一步开发怎么写”。

它只做一件事：

- 把 Claude Code 里**值得学的工程模式**转成 AIRI 的发布边界语言
- 明确本次发布应确认哪些边界，以及哪些边界本次不接收

## 发版适用说明

- 不新增 implementation plan（实现计划）
- 不约束具体开发顺序
- 只约束发布时是否允许继续推进
...
通过：边界定义成立、禁区未触发、发布顺序中的“最小闭环”都可复核。

不通过：任一条出现反向证据或不可复核边界，直接阻断本次发版。

本段的关键核验点：

- runtime shell 与 lane 边界是否被明确化
- verification / repair 是否可验证
- 工具暴露层是否被降为支撑层而非系统中心
- 是否把高复杂度能力错误当成本次发布默认项
