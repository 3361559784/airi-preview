...
| Planning orchestration contract | `planning-orchestration-contract.md`, `src/planning-orchestration/contract.ts`, `src/planning-orchestration/projection.ts`, `src/planning-orchestration/reconciliation.ts`, `src/planning-orchestration/state-transition.ts`, `src/planning-orchestration/host-entrypoint.ts`, `src/planning-orchestration/state-apply.ts`, `src/planning-orchestration/host-runtime.ts`, `src/planning-orchestration/host-runtime-state.ts`, `src/planning-orchestration/host-workflow-caller.ts`, `src/planning-orchestration/runtime-recovery.ts`, `src/planning-orchestration/runtime-replan.ts`, `src/planning-orchestration/runtime-session.ts`, `src/planning-orchestration/session-projection.ts`, `src/planning-orchestration/session-replay.ts`, `src/planning-orchestration/host-planner-recovery-policy.ts`, `src/planning-orchestration/replacement-plan-projection.ts`, `src/planning-orchestration/lane-router.ts`, `src/planning-orchestration/route-projection.ts`, `src/planning-orchestration/workflow-handoff.ts`, `src/planning-orchestration/workflow-mapping.ts`, `src/planning-orchestration/workflow-execution.ts`, `src/planning-orchestration/workflow-session.ts`, `src/planning-orchestration/workflow-evidence.ts`, `src/planning-orchestration/workflow-reconciliation.ts`, `src/coding-runner/transcript-runtime.ts` | `src/planning-orchestration/contract.test.ts`, `src/planning-orchestration/projection.test.ts`, `src/planning-orchestration/reconciliation.test.ts`, `src/planning-orchestration/state-transition.test.ts`, `src/planning-orchestration/host-entrypoint.test.ts`, `src/planning-orchestration/state-apply.test.ts`, `src/planning-orchestration/host-runtime.test.ts`, `src/planning-orchestration/host-runtime-state.test.ts`, `src/planning-orchestration/host-workflow-caller.test.ts`, `src/planning-orchestration/runtime-recovery.test.ts`, `src/planning-orchestration/runtime-replan.test.ts`, `src/planning-orchestration/runtime-session.test.ts`, `src/planning-orchestration/session-projection.test.ts`, `src/planning-orchestration/session-replay.test.ts`, `src/planning-orchestration/host-planner-recovery-policy.test.ts`, `src/planning-orchestration/replacement-plan-projection.test.ts`, `src/planning-orchestration/lane-router.test.ts`, `src/planning-orchestration/route-projection.test.ts`, `src/planning-orchestration/workflow-handoff.test.ts`, `src/planning-orchestration/workflow-mapping.test.ts`, `src/planning-orchestration/workflow-execution.test.ts`, `src/planning-orchestration/workflow-session.test.ts`, `src/planning-orchestration/workflow-evidence.test.ts`, `src/planning-orchestration/workflow-reconciliation.test.ts`, `src/coding-runner/transcript-runtime.test.ts` | Future cross-lane PlanSpec/PlanState authority, optional context projection, evidence reconciliation, deterministic state-transition proposals, host-owned proposal review entrypoint, returned-copy transition apply result, host-owned runtime transition boundary, in-memory current-run state holder, explicit host workflow reconciliation caller, bounded runtime recovery/replan request, host-supplied replacement plan acceptance, host-owned current-run runtime session history, bounded session summary projection in coding context, deterministic session recovery replay rows, host planner recovery policy, replacement plan request projection, deterministic lane-routing classification, route-summary projection, workflow handoff classification, explicit workflow template mapping, explicit mapped workflow execution boundary, workflow-result-to-session reconciliation wiring, workflow-result-to-plan-evidence bridge, and explicit workflow reconciliation summary. No host planner auto-execution. |
...
- lane routing is deterministic classification only; it never executes tools
- lane routing uses registered tool descriptors as source of truth for
  lane compatibility and approval pressure
- route summary projection is bounded current-run guidance and still cannot
  execute routed tools or satisfy proof gates
- workflow handoff classification does not create `WorkflowDefinition` or
  `WorkflowStepTemplate` objects
- workflow mapping requires explicit caller-provided workflow kind and params;
  it does not execute workflows or infer params from plan prose
- mapped workflow execution only delegates already-mapped workflows into the
  existing workflow engine; it defaults `autoApproveSteps` to false and cannot
  satisfy verification gates or mutation proof
- workflow evidence observation only emits `tool_result` observations back to
  the reconciler; it never emits verification-gate or human-approval evidence
- workflow reconciliation requires explicit `PlanState`; without it, execution
  returns skipped reconciliation metadata instead of inventing state
- state transition proposals are descriptive only; they never mutate PlanState
  and must be applied by a future host-owned orchestration boundary
- host entrypoint can accept/reject/request replan, but it still returns only
  an audited decision record and never mutates PlanState
- accepted transition apply returns a copied `nextState`; it does not mutate the
  input state, persist runtime state, execute tools, or satisfy proof gates
- host runtime transition boundary composes review + apply but still does not
  persist state, execute lanes, or register model-visible tools
- runtime state holder persists accepted transitions only inside one current-run
  host instance; it returns defensive copies and still does not write durable
  memory or execute lanes
- explicit host workflow reconciliation caller consumes transition proposals
  from workflow reconciliation but is not a model-visible mutation tool
- runtime recovery may request replanning after blocked/replan-requested
  transitions, but it does not generate replacement PlanSpec
- replacement plan acceptance validates host-supplied PlanSpec and initial
  PlanState, then creates a new current-run runtime holder without durable
  writes
- host runtime session composes initial/replacement runtime holders and
  transition history, but it remains current-run only and cannot execute lanes
- session projection summarizes host runtime session history as bounded guidance
  without exposing session mutation controls
- session recovery replay classifies blocked/rejected/replan/workflow-blocked
  histories as deterministic triage rows, not recovery behavior
- host planner recovery policy maps replay classes to next host action, but it
  does not create replacement plans or mutate sessions
- replacement plan request projection is bounded guidance for a future
  host/planner, not PlanSpec generation
- coding context projection can include that session summary after route summary
  and before workspace/plast-mem/task memory context
- workflow-session wiring executes explicit mapped workflow runs and records
  host-accepted reconciliation transitions in current-run session history
...
| `src/planning-orchestration/contract.ts`, `src/planning-orchestration/projection.ts`, `src/planning-orchestration/reconciliation.ts`, `src/planning-orchestration/state-transition.ts`, `src/planning-orchestration/host-entrypoint.ts`, `src/planning-orchestration/state-apply.ts`, `src/planning-orchestration/host-runtime.ts`, `src/planning-orchestration/host-runtime-state.ts`, `src/planning-orchestration/host-workflow-caller.ts`, `src/planning-orchestration/runtime-recovery.ts`, `src/planning-orchestration/runtime-replan.ts`, `src/planning-orchestration/runtime-session.ts`, `src/planning-orchestration/session-projection.ts`, `src/planning-orchestration/session-replay.ts`, `src/planning-orchestration/host-planner-recovery-policy.ts`, `src/planning-orchestration/replacement-plan-projection.ts`, `src/planning-orchestration/lane-router.ts`, `src/planning-orchestration/route-projection.ts`, `src/planning-orchestration/workflow-handoff.ts`, `src/planning-orchestration/workflow-mapping.ts`, `src/planning-orchestration/workflow-execution.ts`, `src/planning-orchestration/workflow-session.ts`, `src/planning-orchestration/workflow-evidence.ts`, `src/planning-orchestration/workflow-reconciliation.ts`, or plan projection wiring in `src/coding-runner/transcript-runtime.ts` | `pnpm -F @proj-airi/computer-use-mcp exec vitest run src/planning-orchestration/contract.test.ts src/planning-orchestration/projection.test.ts src/planning-orchestration/reconciliation.test.ts src/planning-orchestration/state-transition.test.ts src/planning-orchestration/host-entrypoint.test.ts src/planning-orchestration/state-apply.test.ts src/planning-orchestration/host-runtime.test.ts src/planning-orchestration/host-runtime-state.test.ts src/planning-orchestration/host-workflow-caller.test.ts src/planning-orchestration/runtime-recovery.test.ts src/planning-orchestration/runtime-replan.test.ts src/planning-orchestration/runtime-session.test.ts src/planning-orchestration/session-projection.test.ts src/planning-orchestration/session-replay.test.ts src/planning-orchestration/host-planner-recovery-policy.test.ts src/planning-orchestration/replacement-plan-projection.test.ts src/planning-orchestration/lane-router.test.ts src/planning-orchestration/route-projection.test.ts src/planning-orchestration/workflow-handoff.test.ts src/planning-orchestration/workflow-mapping.test.ts src/planning-orchestration/workflow-execution.test.ts src/planning-orchestration/workflow-session.test.ts src/planning-orchestration/workflow-evidence.test.ts src/planning-orchestration/workflow-reconciliation.test.ts src/coding-runner/transcript-runtime.test.ts` |
...
| plan step is blocked for wrong lane/tool | `src/planning-orchestration/lane-router.ts`, `src/planning-orchestration/lane-router.test.ts`, `src/server/tool-descriptors/*` | workflow engine |
| plan route cannot become workflow handoff | `src/planning-orchestration/workflow-handoff.ts`, `src/planning-orchestration/workflow-handoff.test.ts`, `src/workflows/types.ts` | runner service |
| plan handoff cannot become workflow definition | `src/planning-orchestration/workflow-mapping.ts`, `src/planning-orchestration/workflow-mapping.test.ts`, `src/workflows/types.ts` | runner service |
| mapped plan workflow fails before first step | `src/planning-orchestration/workflow-execution.ts`, `src/planning-orchestration/workflow-execution.test.ts`, `src/workflows/engine.ts` | adding new MCP tools |
| mapped workflow result cannot reconcile plan evidence | `src/planning-orchestration/workflow-evidence.ts`, `src/planning-orchestration/workflow-evidence.test.ts`, `src/planning-orchestration/reconciliation.ts` | verification gate |
| mapped workflow execution has no reconciliation summary | `src/planning-orchestration/workflow-reconciliation.ts`, `src/planning-orchestration/workflow-reconciliation.test.ts`, `src/coding-runner/tool-runtime.ts` | host planner |
| reconciled plan has no next state suggestion | `src/planning-orchestration/state-transition.ts`, `src/planning-orchestration/state-transition.test.ts`, `src/planning-orchestration/reconciliation.ts` | mutating PlanState |
| host cannot audit a transition proposal | `src/planning-orchestration/host-entrypoint.ts`, `src/planning-orchestration/host-entrypoint.test.ts`, `src/planning-orchestration/state-transition.ts` | runner service |
| accepted host transition does not produce next plan state | `src/planning-orchestration/state-apply.ts`, `src/planning-orchestration/state-apply.test.ts`, `src/planning-orchestration/host-entrypoint.ts` | runner service |
| host-reviewed transition cannot become runtime nextState | `src/planning-orchestration/host-runtime.ts`, `src/planning-orchestration/host-runtime.test.ts`, `src/planning-orchestration/state-apply.ts` | model-visible tools |
| accepted transition is not remembered across host runtime calls | `src/planning-orchestration/host-runtime-state.ts`, `src/planning-orchestration/host-runtime-state.test.ts`, `src/planning-orchestration/host-runtime.ts` | workspace memory |
| workflow reconciliation transition does not update host runtime state | `src/planning-orchestration/host-workflow-caller.ts`, `src/planning-orchestration/host-workflow-caller.test.ts`, `src/planning-orchestration/workflow-reconciliation.ts` | coding-runner tool surface |
| blocked or replan-requested plan runtime has no bounded recovery request | `src/planning-orchestration/runtime-recovery.ts`, `src/planning-orchestration/runtime-recovery.test.ts`, `src/planning-orchestration/host-workflow-caller.ts` | auto-generating PlanSpec |
| replacement PlanSpec is accepted without host ownership | `src/planning-orchestration/runtime-replan.ts`, `src/planning-orchestration/runtime-replan.test.ts`, `src/planning-orchestration/runtime-recovery.ts` | model-visible tools |
| plan runtime session loses replacement or transition history | `src/planning-orchestration/runtime-session.ts`, `src/planning-orchestration/runtime-session.test.ts`, `src/planning-orchestration/runtime-replan.ts`, `src/planning-orchestration/host-runtime-state.ts` | durable memory |
| plan runtime session projection acts like a control surface | `src/planning-orchestration/session-projection.ts`, `src/planning-orchestration/session-projection.test.ts`, `src/planning-orchestration/runtime-session.ts` | coding runner tool wiring |
| plan runtime session summary missing from coding context | `src/coding-runner/transcript-runtime.ts`, `src/coding-runner/transcript-runtime.test.ts`, `src/planning-orchestration/session-projection.ts` | adding model-visible session tools |
| mapped workflow result does not update host session state | `src/planning-orchestration/workflow-session.ts`, `src/planning-orchestration/workflow-session.test.ts`, `src/planning-orchestration/workflow-reconciliation.ts`, `src/planning-orchestration/runtime-session.ts` | model-visible session tools |
| plan session recovery class is unknown | `src/planning-orchestration/session-replay.ts`, `src/planning-orchestration/session-replay.test.ts`, `src/planning-orchestration/runtime-session.ts`, `src/planning-orchestration/workflow-session.ts` | runtime recovery changes |
| plan recovery policy loops into automatic replacement plans | `src/planning-orchestration/host-planner-recovery-policy.ts`, `src/planning-orchestration/host-planner-recovery-policy.test.ts`, `src/planning-orchestration/session-replay.ts` | creating planner runtime |
| replacement plan request projection starts generating PlanSpec | `src/planning-orchestration/replacement-plan-projection.ts`, `src/planning-orchestration/replacement-plan-projection.test.ts`, `src/planning-orchestration/host-planner-recovery-policy.ts` | automatic planner runtime |
