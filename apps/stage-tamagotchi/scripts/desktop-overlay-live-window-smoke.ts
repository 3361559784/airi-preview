...
const smokeUrl = `data:text/html;charset=utf-8,${encodeURIComponent(`<!doctype html>
<html>
  <head>
...
async function connectOverlayClient(debugPort: number): Promise<CdpClient> {
  ...
  assert(overlayTarget.webSocketDebuggerUrl, 'APP_START_FAILED: overlay target missing webSocketDebuggerUrl')
...
function selectCandidate(runState: Record<string, unknown>): string {
...
async function main() {
  ...
  try {
    ...
    try {
      ...
      const candidateId = selectCandidate(preClickRunState)
